...
class GeneratorGithub:
    def get_commits_within_dates(self, repository: Repository, start_date, end_date):
        return list(repository.get_commits(since=start_date, until=end_date))

    def print_commit_info(self, list_commit: list[Commit]):
    ...
    def get_changes(self, diff_url: str):
        ...
        for file_dif in list_div:
            # print classs
            file_name = file_dif.find_all("div", class_="file-header")[0].find_all("a")[0].get("title")
            ...
            for span in list_span:
                changes += span.text + "\n"
            dict_changes["list_changes"].append({"file_name": file_name, "changes": changes})
        return dict_changes

    def get_description(self, dict_changes: dict):
    ...
    def generate(self) -> dict:
        ...
        for repository in list_repository:
            ...
            # Fetch commits
            list_commit = self.get_commits_within_dates(
                repository,
                start_date,
                end_date,
            )

            dict_changes = self.get_changes_from_commits(list_commit)
        ...
        if os.path.exists(path_file_post_template):
            ...
            post_template = self.get_description(dict_changes)
